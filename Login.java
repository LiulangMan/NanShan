...
public class Login extends Activity{                 //登录界面活动
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                ...
                case R.id.login_btn_cancle:                             //登录界面的注销按钮
                    cancel();
                    break;
                case R.id.login_text_change_pwd:                             //登录界面的gaimima按钮
    ...
    public void login() {                                              //登录按钮监听事件
        if (isUserNameAndPwdValid()) {
            ...
            if(result==1){                                             //返回1说明用户名和密码均正确
                ...
                Intent intent = new Intent(Login.this,ThridActivity.class) ;    //切换Login Activity至User Activity
                startActivity(intent);
                finish();
                Toast.makeText(Login.this,"登陆成功",Toast.LENGTH_SHORT).show();//登录成功提示
            }else if(result==0){
                Toast.makeText(Login.this,"登陆失败",Toast.LENGTH_SHORT).show();  //登录失败提示
            }
        }
    }
    public void cancel() {           //注销
        if (isUserNameAndPwdValid()) {
            ...
            int result=mUserDataManager.findUserByNameAndPwd(userName, userPwd);
            if(result==1){                                             //返回1说明用户名和密码均正确
                Toast.makeText(Login.this,"注销成功",Toast.LENGTH_SHORT).show();

                mAccount.setText("");
                mUserDataManager.deleteUserDatabyname(userName);
            }else if(result==0){
                Toast.makeText(Login.this,"注销失败",Toast.LENGTH_SHORT).show();  //注销失败提示
            }
        }
