...
public class Resetpwd extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.changemm_layout);
        ...
        if (isUserNameAndPwdValid()) {
            ...
            if(result==1){                                             //返回1说明用户名和密码均正确,继续后续操作
                if(userPwd_new.equals(userPwdCheck)==false){           //两次密码输入不一样
                    Toast.makeText(Resetpwd.this,"两次输入密码不一样",Toast.LENGTH_SHORT).show();
                    return ;
                } else {
                    ...
                    if (flag == false) {
                        Toast.makeText(Resetpwd.this,"FAIL",Toast.LENGTH_SHORT).show();
                    }else{
                ...
                Toast.makeText(Resetpwd.this,"",Toast.LENGTH_SHORT).show();
                return;
            }
        }
    }
    public boolean isUserNameAndPwdValid() {
        ...
        //用户不存在时返回，给出提示文字
        if(count<=0){
            Toast.makeText(Resetpwd.this,"用户不存在",Toast.LENGTH_SHORT).show();
            return false;
        }
        if (mAccount.getText().toString().trim().equals("")) {
            Toast.makeText(Resetpwd.this,"用户名为空",Toast.LENGTH_SHORT).show();
            return false;
        } else if (mPwd_old.getText().toString().trim().equals("")) {
            Toast.makeText(Resetpwd.this,"密码为空",Toast.LENGTH_SHORT).show();
            return false;
